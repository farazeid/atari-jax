        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                if global_step % 100 == 0:
                    # fmt: off
                    mlflow.log_metric("losses/td_loss", jax.device_get(loss), step=global_step)
                    mlflow.log_metric("losses/q_values", jax.device_get(old_val).mean(), step=global_step)
                    mlflow.log_metric("charts/SPS", int(global_step / (time.time() - start_time)), step=global_step)
                    mlflow.log_metric("charts/epsilon", epsilon, step=global_step)
                    # fmt: on
